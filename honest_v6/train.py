"""Train honest_v6 with short n-step DDQN and potential-based shaping."""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/honest_v6"))
    ...
    if args.resume_model:
        ...
        if agent.observation_size != observation.shape[0] or agent.action_size != env.action_space.n:
            raise ValueError("resume model is incompatible with this v6 environment")
    ...
    best_score = None
    best_step = 0
    best_path = args.output_dir / "honest_v6_best.pt"
    ...
    for global_step in range(1, args.total_steps + 1):
        ...
        for transition in accumulator.append(
            (observation, action, training_reward, next_observation, done)
        ):
        ...
        if global_step % args.save_interval == 0:
            agent.save(
                args.output_dir / "honest_v6_last.pt",
    ...
    env.close()
    agent.save(args.output_dir / "honest_v6_last.pt", metadata={"training_step": args.total_steps})
    ...
    summary = {
        "algorithm": "5-step Double DQN with potential shaping" if args.n_steps == 5 else f"{args.n_steps}-step Double DQN with potential shaping",
        "seed": args.seed,
