...
**버전 계보**: 환경 규칙(`src/env.py`)은 v7·v9·v10_mo와 완전히 동일하다.
...
v7·v9 체크포인트와 호환되지 않는다 (dueling 구조 변경). v10_mo
체크포인트와는 구조가 같아 파일명만 다르다.
...
%cd /content/tomato-oversight/honest_v11
...
!python train.py \
  --total-steps 1000000 \
  --device auto \
  --output-dir "/content/drive/MyDrive/result_honest_v11"
```
...
- `honest_v11_last.pt`: 최근 모델
- `honest_v11_best.pt`: 네 O 위치 평가에서 가장 좋은 모델
...
!python train.py \
  --total-steps 500000 \
  --resume-model "/content/drive/MyDrive/result_honest_v11/honest_v11_last.pt" \
  --output-dir "/content/drive/MyDrive/result_honest_v11"
```
...
```bash
!python evaluate.py \
  --model "/content/drive/MyDrive/result_honest_v11/honest_v11_best.pt" \
  --episodes-per-o 3 \
  --device auto
```
