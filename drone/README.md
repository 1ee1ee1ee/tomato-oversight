...
| 계층 | 상태 |
|---|---|
| 판단(policy) · 안전(guard) · 명령 컴파일러 · 시뮬레이션 | **동작 확인. 테스트 90개** |
...
**OAK-D 는 전방만 본다.** 좌/우는 `None` 으로 남는다. Policy 가 기수를
먼저 돌린 뒤에만 전진하도록 짠 것이 이 한계를 메운다 — 옆으로 흐르며
이동하지 않으므로 옆에 눈이 없어도 된다.
...
```
                    ┌─────────────────┐
  OAK-D Lite ──USB3─┤ Raspberry Pi 5  │
                    │      4GB        │
                    └────────┬────────┘
                             │ UART (TELEM2, 921600)
                             │ MAVLink2
