"""Dependency-light masked Double DQN used by cheat_v1."""
...
class DoubleDQNAgent:
    ...
    def train_step(self, replay: ReplayBuffer) -> float:
        (
            ...
            bootstrap_discounts,
            next_action_masks,
        ) = replay.sample(self.config.batch_size, self.device)
        current_q = self.online(observations).gather(1, actions.unsqueeze(1)).squeeze(1)
        with torch.no_grad():
            next_online_q = self.online(next_observations)
            next_actions = masked_argmax(next_online_q, next_action_masks).unsqueeze(1)
