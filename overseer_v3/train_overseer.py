"""Train the overseer DQN (regime 0..4) against frozen robots.

Regimes are the v3 information ablation (see ``src/env.py`` for the full table):
0 = timing+audit only, 1 = +apparent log, 2 = +behaviour incl. watering but no
apparent log (v2's regime 2), 3 = +behaviour with watering withheld, 4 = all.
Every regime shares one observation length, so the network size is constant and
a regime gap measures information rather than capacity.
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/overseer_v3"))
