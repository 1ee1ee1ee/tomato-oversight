...
마스킹은 **차원을 자르는 게 아니라 상수로 덮는 것**이다. 가려진 입력은 순전파에서 `0 × w = 0`이라
기여가 없고, 그래디언트도 `δ × 0 = 0`이라 그 열의 가중치는 **초기값에 영원히 고정**된다
(실측: 양쪽 다 가린 열의 `max|ΔW| = 0.00e+00`). 정보량으로는 차원 제거와 동등하면서
파라미터 수만 불변이다.

대가가 하나 있다. v2에서는 체제를 잘못 지정하면 21 vs 30 차원 불일치로 **에러가 났지만**,
v3는 전부 36이라 **조용히 돌아간다** — 체제 0 체크포인트를 `--regime 4`로 평가하면 새로
살아난 열이 초기값 난수 가중치와 곱해져 쓰레기를 낸다. 그래서 학습 시 체크포인트에 regime을
기록하고(`train_overseer.py`), 평가 도구가 `check_checkpoint_regime()`으로 불일치를 거부한다.
v2 체크포인트(21/30차원)도 여기서 명확한 메시지와 함께 막힌다.
