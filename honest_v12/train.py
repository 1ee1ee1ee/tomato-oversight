"""Train honest_v12 with masked 5-step DDQN and potential shaping.

Identical to honest_v11 except that training episodes start from randomized
mid-episode states (``--no-random-reset`` restores the honest_v11 behaviour).
Evaluation and best-model selection always use the standard step-0 start, so
the reported metrics stay comparable with honest_v11.
"""
...
def state_potential(
    ...
    moisture_start = 2
    alive_start = moisture_start + tomato_count
    moisture = observation[moisture_start:alive_start]
    alive = observation[alive_start:alive_start + tomato_count]
    return float(moisture_weight * moisture.sum() + alive_weight * alive.sum())
...
def potential_shaping_reward(
    ...
    current_potential = state_potential(
        observation, tomato_count, moisture_weight, alive_weight
    )
    next_potential = 0.0 if done else state_potential(
        next_observation, tomato_count, moisture_weight, alive_weight
    )
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/honest_v12"))
    ...
    best_score = None
    best_step = 0
    best_path = args.output_dir / "honest_v12_best.pt"
    ...
    for global_step in range(1, args.total_steps + 1):
        ...
        shaping_reward = potential_shaping_reward(
            ...
            moisture_weight=args.moisture_potential_weight,
        ...
        if global_step % args.save_interval == 0:
            agent.save(
                args.output_dir / "honest_v12_last.pt",
    ...
    env.close()
    agent.save(args.output_dir / "honest_v12_last.pt", metadata={"training_step": args.total_steps})
