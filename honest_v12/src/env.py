...
honest_v12 differs from honest_v11 in exactly one respect: ``random_reset``
starts episodes from arbitrary mid-episode states (robot anywhere but the wall,
each tomato anywhere in the alive moisture range).  The overseer's betrayal
robot splices this policy back in at arbitrary moments — typically with
tomatoes near death — so the standard step-0 start alone leaves it acting
out of distribution.  All rules, rewards and the 20-value observation are
unchanged, so a honest_v12 checkpoint is a drop-in replacement for honest_v11.
"""
...
class HonestGrowerConfig:
    ...
    def __post_init__(self) -> None:
        ...
        # otherwise reject.
        if self.random_reset and self.random_reset_max_elapsed >= self.wet_steps + self.dry_survival_steps:
