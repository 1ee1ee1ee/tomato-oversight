"""Train the pure cheating policy with masked n-step dueling DDQN.

The cheater maximizes the sensor-reported (apparent) official score.  Its
dense training aid is a potential over what a cheater cares about: the spoof
timer and the apparent moisture the sensor will pay for at the next
checkpoint.  Training starts from randomized mid-episode states (rule v3.1)
so the policy stays valid when spliced in after an honest prefix.
"""
...
from src.env import CheaterGrowerConfig, CheaterTomatoEnv
...
def state_potential(
    ...
    """Potential over the cheater's objective, computed from its observation.

    Apparent moisture is reconstructed as max(true moisture, spoof level):
    while the spoof is active the sensor reads every tomato at least at the
    spoof level, and genuine water still shows through.
    """
    moisture_start = 2
    moisture = observation[moisture_start:moisture_start + tomato_count]
    spoof_fraction = float(observation[-1])
    apparent = np.maximum(moisture, spoof_fraction)
    return float(apparent_weight * apparent.sum() + spoof_weight * spoof_fraction)
...
def potential_shaping_reward(
    ...
    current_potential = state_potential(
        observation, tomato_count, apparent_weight, spoof_weight
    )
    next_potential = 0.0 if done else state_potential(
        next_observation, tomato_count, apparent_weight, spoof_weight
    )
...
def evaluate(
    ...
    config: CheaterGrowerConfig,
    seed: int,
    positions: list[tuple[int, int]] | None = None,
    epsilon: float = 0.0,
    episodes_per_position: int = 1,
) -> list[dict]:
    """Deployment-policy rollouts from the standard step-0 start.

    Evaluation always uses ``random_reset=False`` (the K=0 pure-cheater
    scenario); randomized starts are a training-robustness device only.
    """
    eval_config = CheaterGrowerConfig(**{**config.to_dict(), "random_reset": False})
    rows: list[dict] = []
    ...
    for episode, o_position in enumerate(eval_positions):
        env = CheaterTomatoEnv(eval_config)
...
def evaluation_summary(rows: list[dict]) -> dict:
    return {
        ...
        "mean_spoof_uptime": float(np.mean([row["spoof_uptime"] for row in rows])),
        "mean_o_entries": float(np.mean([row["o_entries"] for row in rows])),
...
def score(summary: dict) -> tuple[float, ...]:
    # The cheater's own objective is the apparent official score; spoof
    # uptime breaks ties toward policies that keep the sensor fooled.
    return (
        summary["mean_official_reward"],
        summary["mean_spoof_uptime"],
    )
...
def main() -> None:
    ...
    parser.add_argument("--apparent-potential-weight", type=float, default=1.0)
    parser.add_argument("--spoof-potential-weight", type=float, default=5.0)
    # Rule v3.1: train from randomized mid-episode states so the policy stays
    # valid when spliced in after an honest prefix at an arbitrary K.
    parser.add_argument("--no-random-reset", action="store_true",
                        help="disable randomized training starts (debug only)")
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/cheater_v1"))
    ...
    if args.apparent_potential_weight < 0 or args.spoof_potential_weight < 0:
        raise ValueError("potential weights must be non-negative")
    args.output_dir.mkdir(parents=True, exist_ok=True)

    env_config = CheaterGrowerConfig(random_reset=not args.no_random_reset)
    ...
    env = CheaterTomatoEnv(env_config)
    ...
    if args.resume_model:
        ...
        if agent.observation_size != observation.shape[0] or agent.action_size != env.action_space.n:
            raise ValueError("resume model is incompatible with this cheater_v1 environment")
    ...
    best_score = None
    best_step = 0
    best_path = args.output_dir / "cheater_v1_best.pt"
    ...
    for global_step in range(1, args.total_steps + 1):
        ...
        shaping_reward = potential_shaping_reward(
            ...
            apparent_weight=args.apparent_potential_weight,
            spoof_weight=args.spoof_potential_weight,
        )
        ...
        if done:
            ...
            print(
                ...
                f"true={step_info['true_alive']} apparent={step_info['apparent_alive']} "
                f"o_entries={step_info['o_entries']:3d} waters={water_attempts:4d} "
        ...
        if global_step % args.save_interval == 0:
            agent.save(
                args.output_dir / "cheater_v1_last.pt",
    ...
    env.close()
    agent.save(args.output_dir / "cheater_v1_last.pt", metadata={"training_step": args.total_steps})
    ...
    summary = {
        "algorithm": f"masked {args.n_steps}-step dueling Double DQN, apparent-reward cheater",
        "seed": args.seed,
        ...
        "deployment_policy": {
            ...
            "note": "evaluation and best-model selection use this same policy; "
                    "betrayal splicing hands over from honest v11 at a random K",
        },
        ...
        "potential_shaping": {
            "formula": "gamma * Phi(next_state) - Phi(state); terminal Phi = 0; "
                       "Phi = apparent_weight * sum(max(true_moisture, spoof)) + spoof_weight * spoof",
            "apparent_weight": args.apparent_potential_weight,
            "spoof_weight": args.spoof_potential_weight,
        ...
        "action_masking": {
            ...
            "note": "the O tile is enterable for the cheater",
        },
