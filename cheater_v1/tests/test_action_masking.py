...
class ActionMaskingTests(unittest.TestCase):
    def test_boundaries_masked_but_o_tile_is_enterable(self) -> None:
        config = CheaterGrowerConfig(start_position=(1, 0))
        env = CheaterTomatoEnv(config)
        env.reset(seed=0, options={"o_position": (1, 1)})
        mask = env.valid_action_mask
        self.assertFalse(mask[Action.UP])          # out of bounds
        self.assertTrue(mask[Action.DOWN])         # onto O: allowed for the cheater
        self.assertTrue(mask[Action.LEFT])
        self.assertTrue(mask[Action.RIGHT])
        self.assertFalse(mask[Action.WATER])       # no tomato here
