...
class HonestGrowerConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.sync_reset_min_elapsed <= self.random_reset_max_elapsed - self.sync_reset_jitter:
            raise ValueError(
                "sync_reset_min_elapsed must leave room for the jitter below "
                "random_reset_max_elapsed"
            )
        ...
        # Injecting a dead tomato would be a different (and unrescuable) event.
        if self.handoff_prob > 0.0 and self.handoff_max_elapsed + self.handoff_jitter >= self.death_at:
...
class HonestTomatoEnv(gym.Env[np.ndarray, int]):
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        if self.config.random_reset:
            ...
            if self.np_random.random() < self.config.sync_reset_prob:
                ...
                # honest_v15 floors the base so the draw lands in the band real
                # handoffs actually occupy instead of mostly below it.
                base = int(self.np_random.integers(
                    self.config.sync_reset_min_elapsed,
                    self.config.random_reset_max_elapsed - self.config.sync_reset_jitter + 1,
                ))
    ...
    def _handoff_due(self) -> bool:
        if self.config.handoff_prob <= 0.0:
            return False
        if self.step_count % self.config.handoff_interval != 0:
            return False
    ...
    def _apply_handoff(self) -> None:
        ...
        jitter = self.np_random.integers(
            0, self.config.handoff_jitter + 1, size=5
        ).astype(np.int32)
        target = np.minimum(base + jitter, self.config.death_at - 1)
        elapsed = np.maximum(self.elapsed_since_water, target)
