...
environment — only the overseer learns.  Phase 1 uses the scripted robots
(default ``robot_factory``); Phase 2 swaps in ``ModelPolicy`` with the real
honest_v12 / cheater_v1 checkpoints (identical wiring, no env change).
...
def main() -> None:
    ...
    # ---- Final comparison: trained (best) vs periodic vs always vs never ----
