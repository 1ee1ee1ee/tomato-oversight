...
Two cheater schedulers live here, not in the world:

* ``BetrayalPolicy`` — rule v3.1's one-way switch: honest with O masked before
  the switch step ``K`` (the pre-K phase is *literally* honest, zero
  discrepancy), then the cheater policy with O enabled from ``K`` on.
  ``K = None`` is a pure-honest episode; ``K = 0`` a pure cheater.
* ``AdaptiveBetrayalPolicy`` — a two-way, survival-aware cheater: it moves
  back and forth between honest and cheating modes at a target ``cheat_fraction``
  and, with survival enforcement on, drops into an honest rescue patrol whenever
  a living tomato nears death.  Because it keeps the tomatoes alive, it produces
  a *moisture* lie (apparent > true) without ever producing a spoofed corpse —
  which is exactly why the overseer catches on moisture mismatch, not deaths.
"""
